...
@Service
@Component
public class ServiceMarginYearTillDate {
	...
	public  List<DtoTableRow> perFormIteration(List<Object[]> l1,String m_strCurrencySymbolFormat,
			int staticIntCurrentDate,int n_IntCurrentMonthNumber,String m_strPeriodictype,String m_strStatQarter,String m_strStatYTD){
		List<DtoTableRow> tableData = new ArrayList<>();
		
		int n_Intmonths=12;
		if(m_strPeriodictype.equals("FY")) {
 	       if(m_strStatQarter.equals("All")==false) {   n_Intmonths=3;  }
	    }else if(m_strPeriodictype.equals("YTD")) {
	 	   n_Intmonths=n_IntCurrentMonthNumber;  
	 	}
		
		System.out.println("Number which will help to get average==> "+n_IntCurrentMonthNumber+" Division Number="+n_Intmonths);
			   ...
			   DtoTableRow row1 = new DtoTableRow();
			    ...
			    try {
			        ...
			        try {
			        	...
			        	m_intContractValue=repositoryEntityContractDetailsnewMaster.returnContractValueFromCustomerIdAndYearOnly(String.valueOf(m_strRmData[1]),m_strStatYTD,m_strStatYTD);
			        	if(m_intContractValue==null || m_intContractValue.equals("") || m_intContractValue.equals("0")) {
			        		m_intContractValue=repositoryEntityContractDetailsnewMaster.returnContractValueFromCustomerIdAndOneStartYearParam(String.valueOf(m_strRmData[1]),m_strStatYTD);
			        		if(m_intContractValue==null || m_intContractValue.equals("") || m_intContractValue.equals("0")) {
				        		m_intContractValue=repositoryEntityContractDetailsnewMaster.returnContractValueFromCustomerIdAndOneEndYearParam(String.valueOf(m_strRmData[1]),m_strStatYTD);
				        	}
			        	}
    ...
    public ByteArrayInputStream getActualData(String m_strCurrencySymbolFormat, int staticIntCurrentDate, int staticIntCurrentMonthNumber,
    		String m_strStatPeriodicType,String m_strStatQarter,String m_strStatYTD) throws IOException {
		String m_strQuery="";
		m_strQuery=this.beanApproximationMargin.getM_strQueryToDownload();
		
		System.out.println("QUERY TO DOWNLOAD EXCEL REPORT= " +m_strQuery);
		...
		if(l1.isEmpty()==false) {
	    	tableData=this.perFormIteration(l1,m_strCurrencySymbolFormat,staticIntCurrentDate,staticIntCurrentMonthNumber,m_strStatPeriodicType,m_strStatQarter,m_strStatYTD);
	    }
